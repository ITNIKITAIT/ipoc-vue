...
const raiseDeadlineInput =
  useTemplateRef<HTMLInputElement>("raiseDeadlineInput");

function openDeadlinePicker() {
  const el = raiseDeadlineInput.value;
  if (!el) return;
...
const INVESTOR_OPTIONS: {
  ...
  {
    value: "none",
    label: "No vesting \u2014 tokens released immediately at TGE",
  },
...
<template>
  <form class="text-white flex flex-col gap-6" @submit="onSubmit">
    ...
    <p class="text-base font-medium text-white">
      Define how much you're raising, how tokens are priced, and how value is
      distributed. This information determines how investors fund your project
      and how tokens are issued.
    </p>
    ...
    <div class="flex flex-col gap-[22px]">
        ...
        <div class="flex flex-col gap-[9px] items-end">
          <div class="relative w-full">
            <UiInput
              ...
              class="pr-24" />
            <span
              class="absolute right-4 top-1/2 -translate-y-1/2 text-sm text-[#d9d8d8] pointer-events-none"
              >USD / USDC</span
            >
          </div>
          <p
            v-if="errors.fundingGoal"
            class="text-red-400 text-xs text-right w-full">
            {{ errors.fundingGoal }}
          </p>
          <p v-else class="text-xs text-[#d9d8d8] text-right w-full">
            The total amount you want to raise before launch.
          </p>
        </div>
      </div>
      ...
      <div class="flex flex-col gap-2">
        ...
        <div class="flex flex-col gap-[9px] items-end">
          <button
            ...
            <span
              :class="
                raiseDeadline ? 'text-white flex-1' : 'text-[#d9d8d8] flex-1'
              ">
              {{
                raiseDeadline
                  ? formatDeadlineDisplay(raiseDeadline)
                  : "Select date"
              }}
            </span>
            <ChevronDown :size="20" class="text-[#d9d8d8] shrink-0" />
            <input
              ref="raiseDeadlineInput"
          ...
          <p
            v-if="errors.raiseDeadline"
            class="text-red-400 text-xs text-right w-full">
            {{ errors.raiseDeadline }}
          </p>
          <p v-else class="text-xs text-[#d9d8d8] text-right w-full">
            If your raise does not reach 100% by this date, investors can
            redirect or withdraw their funds.
          </p>
        </div>
      </div>
      ...
      <div class="flex flex-col gap-2">
        ...
        <div class="flex flex-col gap-[9px] items-end">
          ...
          <p class="text-xs text-[#d9d8d8] text-right w-full">
            Token Price (auto-calculated)* that updates live as the creator
            enters their raise amount and fundraise allocation %
          </p>
        </div>
      </div>
    </div>
    ...
    <div class="flex flex-col gap-4">
      ...
      <div class="flex gap-10 items-stretch">
        <div class="flex-1 bg-white/25 rounded-[16px] p-8 flex flex-col gap-4">
          <p class="text-base font-medium text-white">
            Funding outcome (auto-calculated)
          </p>
          <p class="text-2xl font-semibold text-white">
            {{
              values.fundingGoal && values.fundingGoal > 0
                ? `${(publicSaleTokens / 1_000_000).toFixed(0)}M tokens`
                : "\u2014"
            }}
          </p>
        </div>
        <div class="flex-1 bg-white/25 rounded-[16px] p-8 flex flex-col gap-4">
          ...
          <p class="text-2xl font-semibold text-white">
            {{
              values.fundingGoal && values.fundingGoal > 0
                ? `$${Number(values.fundingGoal).toLocaleString()}`
                : "\u2014"
            }}
          </p>
        </div>
    ...
    <div class="flex flex-col gap-8">
      <h2
        class="text-[32px] font-semibold text-white leading-[1.28] tracking-[-0.16px]">
        Token Overview
      </h2>
    ...
    <section class="flex flex-col gap-4">
      <div class="flex items-center gap-4">
        <h2
          class="text-[18px] font-semibold uppercase tracking-[0.5px] text-[#d9d8d8]">
          Investor Vesting
        </h2>
        <span
          class="bg-[#2bced4] rounded-[8px] px-4 py-1 text-sm text-[#002147]"
          >Recommended</span
        >
      </div>
      <div class="flex gap-4 items-center">
        ...
        <p class="text-xl text-[#d9d8d8]">
          Choose how investors receive their tokens after the fundraise.
        </p>
      </div>
      <div class="flex flex-col gap-4 pl-6">
        <label
          v-for="opt in INVESTOR_OPTIONS"
          :key="opt.value"
          class="flex items-center gap-[7px] cursor-pointer">
          <input
            type="radio"
            class="sr-only"
            :value="opt.value"
            :checked="values.investorVesting === opt.value"
            @change="setFieldValue('investorVesting', opt.value)" />
          <span
            :class="[
              'size-6 rounded-full border-2 shrink-0 flex items-center justify-center',
              values.investorVesting === opt.value
                ? 'border-[#156bb7]'
                : 'border-[#555]',
            ]">
            <span
              v-if="values.investorVesting === opt.value"
              class="size-3 rounded-full bg-[#156bb7]" />
          </span>
          <span class="text-base text-white leading-[1.7]">{{
            opt.label
          }}</span>
          <span
            v-if="opt.badge"
            :class="[
              'text-sm px-4 py-1 rounded-[8px] shrink-0',
              opt.badge.bg,
              opt.badge.textColor,
            ]"
            >{{ opt.badge.text }}</span
    ...
    <div class="flex flex-col gap-4">
      ...
      <CreateProjectVestingTimelineChart
        :investor-vesting="values.investorVesting"
        :team-vesting="values.teamVesting" />
    </div>
    ...
    <section class="flex flex-col gap-4">
      ...
      <div class="flex flex-col gap-2">
        <div class="flex gap-4 items-center">
          <span class="size-[15px] bg-[#1a7c7f] rounded-[4px] shrink-0" />
          <p class="text-xl text-[#d9d8d8]">
            Team tokens must be locked to build trust with investors.
          </p>
        </div>
        <div class="flex gap-4 items-center">
          <span class="size-[15px] opacity-0 shrink-0" />
          <p class="text-base font-medium text-[#d9d8d8]">
            Minimum 6-month waiting period required
          </p>
        </div>
      </div>
      <div class="flex flex-col gap-4 pl-6">
        <label
          v-for="opt in TEAM_OPTIONS"
          :key="opt.value"
          class="flex items-center gap-[7px] cursor-pointer">
          <input
            type="radio"
            class="sr-only"
            :value="opt.value"
            :checked="values.teamVesting === opt.value"
            @change="setFieldValue('teamVesting', opt.value)" />
          <span
            :class="[
              'size-6 rounded-full border-2 shrink-0 flex items-center justify-center',
              values.teamVesting === opt.value
                ? 'border-[#156bb7]'
                : 'border-[#555]',
            ]">
            <span
              v-if="values.teamVesting === opt.value"
              class="size-3 rounded-full bg-[#156bb7]" />
          </span>
          <span class="text-base text-white leading-[1.7]">{{
            opt.label
      ...
      <p class="text-base font-medium text-[#d9d8d8]">
        Team tokens must be locked to ensure long-term commitment and investor
        trust.
      </p>
      <div class="text-sm text-[#d9d8d8] flex flex-col gap-0">
        <p>
          <strong class="font-bold">A minimum 6-month</strong> cliff is required
          by IPOC.
        </p>
    ...
    <div class="flex items-end justify-between pb-[72px]">
      ...
      <div class="flex items-center gap-4 mt-10">
        ...
        <button
          type="submit"
          class="relative flex items-center gap-2 rounded-full border border-[#156bb7] bg-white px-6 py-3 text-[18px] font-semibold uppercase text-black overflow-hidden shadow-[inset_0px_4px_8px_0px_rgba(0,0,0,0.25)] transition-opacity hover:opacity-80">
          Next
          <ArrowRight :size="24" />
        </button>
      </div>
