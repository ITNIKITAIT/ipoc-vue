...
<template>
  <div
    ...
    :style="{
      background:
        'linear-gradient(90deg, rgba(0,0,0,0.38) 0%, rgba(0,0,0,0.38) 100%), linear-gradient(180deg, #002147 0%, #000 100%)',
    }">
    <p
    ...
    <div class="flex gap-6 items-center">
        ...
        <div v-for="(row, i) in ALLOCATION" :key="row.name">
          <div class="flex items-center justify-between px-6 py-4">
            <div class="w-[64px] pr-4">
              <span
                class="inline-block w-[45px] h-[18px] rounded-[4px]"
                :style="{ backgroundColor: row.color }" />
            </div>
            <span class="w-[200px] text-base font-medium text-white">{{
              row.name
            }}</span>
            <span class="flex-1 text-center text-base font-medium text-white"
              >{{ row.pct }}%</span
            >
            <span
              class="flex-1 text-right text-base font-medium text-white tabular-nums"
              >{{ row.tokens.toLocaleString() }}</span
            >
          </div>
          <div
            v-if="i < ALLOCATION.length - 1"
            class="h-px w-full"
            style="background: rgba(255, 255, 255, 0.12)" />
        </div>
      </div>
      ...
      <div class="shrink-0 w-[340px] h-[340px] relative">
        <svg viewBox="0 0 340 340" class="block size-full">
          <circle
        ...
        <div
          ...
          <span
            class="text-[18px] !font-roboto font-semibold uppercase tracking-[0.5px] text-white leading-[1.5]"
            >Total Supply</span
          >
          <span
            class="text-[20px] !font-roboto text-white leading-[1.5] whitespace-nowrap"
            >1 Billion tokens</span
          >
          <span
            class="text-[20px] !font-roboto text-white leading-[1.5] whitespace-nowrap"
            >(fixed for all projects)</span
          >
        </div>
      </div>
    </div>
